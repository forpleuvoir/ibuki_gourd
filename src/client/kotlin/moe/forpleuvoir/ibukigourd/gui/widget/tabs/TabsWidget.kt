...
class TabsWidget(
    ...
    val direction: moe.forpleuvoir.ibukigourd.gui.base.Direction = moe.forpleuvoir.ibukigourd.gui.base.Direction.Top
) : AbstractElement() {
    ...
    override val layout: Layout = object : Layout {
        ...
        override fun arrange(
            ...
            margin: moe.forpleuvoir.ibukigourd.gui.base.Margin,
            padding: moe.forpleuvoir.ibukigourd.gui.base.Margin
        ): Size<Float>? {
            if (elements.isEmpty()) return null

            val offset = when (direction) {
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Top    -> vertex(0, 1f, 0)
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Bottom -> vertex(0, -1f, 0)
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Left   -> vertex(1f, 0, 0)
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Right  -> vertex(-1f, 0, 0)
            }

            val alignment = when (direction) {
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Top    -> PlanarAlignment.BottomLeft(Orientation.Horizontal)
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Bottom -> PlanarAlignment.TopLeft(Orientation.Horizontal)
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Left   -> PlanarAlignment.TopRight(Orientation.Vertical)
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Right  -> PlanarAlignment.TopLeft(Orientation.Vertical)
            }
            ...
            val contentRect = when (direction) {
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Top    -> Rect(vertex(4f, 0, 0), transform.width, size.height)
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Bottom -> Rect(vertex(4f, transform.height - size.height, 0), transform.width, size.height)
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Left   -> Rect(vertex(0, 4f, 0), size.width, transform.height)
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Right  -> Rect(vertex(transform.width - size.width, 4f, 0), size.width, transform.height)
            }
    ...
    override fun contentBox(isWorld: Boolean): Rectangle<Vector3<Float>> {
        val top =
            (if (isWorld) transform.worldTop + padding.top else padding.top) + if (direction == moe.forpleuvoir.ibukigourd.gui.base.Direction.Top) tabSize.height - 3 else 0f
        val bottom =
            (if (isWorld) transform.worldBottom - padding.bottom else transform.height - padding.bottom) - if (direction == moe.forpleuvoir.ibukigourd.gui.base.Direction.Bottom) tabSize.height - 3 else 0f
        val left =
            (if (isWorld) transform.worldLeft + padding.left else padding.left) + if (direction == moe.forpleuvoir.ibukigourd.gui.base.Direction.Left) tabSize.width - 3 else 0f
        val right =
            (if (isWorld) transform.worldRight - padding.right else transform.width - padding.right) - if (direction == moe.forpleuvoir.ibukigourd.gui.base.Direction.Right) tabSize.width - 3 else 0f
        return Rect(
...
internal fun TabsWidget.tabButton(
    ...
    return addTabElement(object : ButtonWidget({
        this.switchTab(tab)
        NextAction.Cancel
    }, { NextAction.Cancel }, { (current == tab).pick({ backgroundColor() }, { inactiveColor() }) }, 0f, Theme.BUTTON.TEXTURE, null, null,
        moe.forpleuvoir.ibukigourd.gui.base.Padding(2), null
    ) {

        override fun onRenderBackground(renderContext: RenderContext) {
            val (active, inactive) = when (direction) {
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Top    -> TAB_ACTIVE_TOP to TAB_INACTIVE_TOP
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Bottom -> TAB_ACTIVE_BOTTOM to TAB_INACTIVE_BOTTOM
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Left   -> TAB_ACTIVE_LEFT to TAB_INACTIVE_LEFT
                moe.forpleuvoir.ibukigourd.gui.base.Direction.Right  -> TAB_ACTIVE_RIGHT to TAB_INACTIVE_RIGHT
            }
...
fun ElementContainer.tabs(
    ...
    direction: moe.forpleuvoir.ibukigourd.gui.base.Direction = moe.forpleuvoir.ibukigourd.gui.base.Direction.Top,
