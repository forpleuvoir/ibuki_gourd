 ...
 *
 * 坐标系（[xRange] / [yRange] 决定可见范围）：
 * - x 轴是 [xRange]、y 轴是 [yRange]，两者各自的 `0f` / `1f` 围出的**单位正方形**用
 *   [BezierCurvePlotTokens.UnitArea] 填充，正方形之外的带留在 [BezierCurvePlotTokens.Background]
 *   上：y 带里会出现回弹 / 过冲的控制点，x 带只用来给 `x = 0f` / `x = 1f` 的控制点留显示空间
 *   —— 控制点 x 恒夹在 [BezierCurvePlotDefaults.ValidXRange]，不会进入 x 带；
 * - 画布四周留 [BezierCurvePlotDefaults.EdgePadding]，控制点贴到边界时仍完整可见。
 *
 * 手势：**命中式拾取**——只有按在控制点的命中半径内才开始拖拽，点空白处不动（避免把曲线误拽变形）；
 * 拖动时 x 夹在 [BezierCurvePlotDefaults.ValidXRange]、y 夹在 [yRange]（指针进 x 越界带时控制点停在
 * `0f` / `1f`）；拖动过程中支持三个修饰键：
 ...
 * @param xRange x 轴显示区间（须递增）；控制点 x 的取值恒为 [BezierCurvePlotDefaults.ValidXRange]，
 *   本参数只决定 `0f..1f` 显示在画布的哪一段
...
fun BezierCurvePlot(
    ...
    xRange: ClosedFloatingPointRange<Float> = BezierCurvePlotDefaults.XRange,
    ...
    require(xRange.endInclusive > xRange.start) { "xRange 必须递增，实际为 $xRange" }
    ...
    // 控制点 x 是曲线定义域，越界会让曲线不再是函数：绘制与拖拽基准都用收敛后的值，画布自身保持自洽
    val plotValue = value.normalize()

    // 手势协程要读到最新值与最新回调，避免 pointerInput 因 lambda / 值变化而重启（重启会打断正在进行的拖拽）
    val currentValue by rememberUpdatedState(plotValue)
    ...
    Canvas(
        modifier
            ...
            // 拖拽：按下时拾取控制点，随后跟随指针；x 夹 ValidXRange、y 夹 yRange，Alt 吸附 / Shift 锁角 / Ctrl 微调
            .pointerInput(
                enabled && interactive,
                xRange.start,
                xRange.endInclusive,
                ...
                awaitEachGesture {
                    val space = plotSpace(size.width, size.height, xRange, yRange, edgePaddingPx)
        ...
        val space = plotSpace(size.width.toInt(), size.height.toInt(), xRange, yRange, edgePaddingPx)
        val easing = plotValue.toEasing()
        ...
        clipRect(0f, 0f, size.width, size.height) {
            ...
            val lineEnd0 = space.handleCenter(Handle.P1, plotValue)
            val lineStart1 = Offset(space.screenX(1f), space.screenY(1f))
            val lineEnd1 = space.handleCenter(Handle.P2, plotValue)
            ...
            val rawCenters = arrayOf(
                space.handleCenter(Handle.P1, plotValue),
                space.handleCenter(Handle.P2, plotValue),
            )
            val tops = Array(2) { i ->
                Offset(
 ...
 * 画布坐标 ↔ 曲线坐标的换算。
 *
 * x / y 两轴各自按 [xRange] / [yRange] 线性映射到画布可用区域（两轴区间跨度可以不同）。
 *
 * @param width 画布宽（像素）
 * @param height 画布高（像素）
 * @param xRange x 轴显示区间
...
private class PlotSpace(
    width: Float,
    height: Float,
    private val xRange: ClosedFloatingPointRange<Float>,
    private val yRange: ClosedFloatingPointRange<Float>,
    edgePadding: Float,
) {

    private val xSpan = xRange.endInclusive - xRange.start
    ...
    /**
     * x 的取值区间：恒为 [BezierCurvePlotDefaults.ValidXRange]（曲线定义域），与显示区间 [xRange] 无关
     * —— 显示可以带越界带，取值不可以。
     */
    private val xValueRange: ClosedFloatingPointRange<Float> = BezierCurvePlotDefaults.ValidXRange

    /** x 取值区间的下界。 */
    val minX: Float get() = xValueRange.start

    /** x 取值区间的上界。 */
    val maxX: Float get() = xValueRange.endInclusive
    ...
    /** 曲线 x（[xRange] 内）→ 画布 x。 */
    fun screenX(x: Float): Float = left + (x - xRange.start) / xSpan * innerWidth
    ...
    /** 画布 x → 曲线 x（夹在 [xValueRange] 内；指针落在越界带里时停在 `0f` / `1f`）。 */
    fun curveX(px: Float): Float =
        (xRange.start + (px - left) / innerWidth * xSpan).coerceIn(xValueRange.start, xValueRange.endInclusive)
     ...
     * （1 = 原长度；系数乘在方向向量上）。
     *
     * x 范围是取值区间 [xValueRange]（不是显示区间），y 范围是 [yRange]。
     * 方向在该轴上分量为 0 时该轴不构成限制。
     */
    fun maxScale(anchor: Offset, directionX: Float, directionY: Float): Float {
        var max = Float.MAX_VALUE
        if (directionX > 1e-6f) max = minOf(max, (xValueRange.endInclusive - anchor.x) / directionX)
        else if (directionX < -1e-6f) max = minOf(max, (xValueRange.start - anchor.x) / directionX)
...
private fun plotSpace(
    ...
    edgePadding: Float,
): PlotSpace = PlotSpace(width.toFloat(), height.toFloat(), xRange, yRange, edgePadding)
...
private fun movedBezier(
    ...
    val x = (if (snap) snapTo(rawX, step) else rawX).coerceIn(space.minX, space.maxX)
