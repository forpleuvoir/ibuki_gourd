...
- **组件**：`Button` / `ColorButton`（Button 薄包装：底色由调用方给定**具体颜色**而非主题槽位，内容色按底色亮度取纯黑/纯白，自有 `color_button` meta 段）/ `Surface` / `Switch` / `Text` / `Slider` / `RadioButton`（Button 薄包装：按 index/count/RTL 解析 left/center/right/single 分段纹理，`RadioButtonGroup` 经 `item { }` DSL 自动编号）/ `RadioButtonGroup`；`Slider` 为「凹槽轨道 + 按进度裁剪的填充」两张精灵叠放，`label` 槽内容画两遍、各裁到进度边界内外以分用两种内容色；`NumberSlider.kt` 在 Slider 之上提供类型化入口（Int/Long/Float/Double/Duration/Percent，双向换算 + 数值标签）；`TextField` 为凹槽背景精灵 + `BasicTextField`（聚焦/错误描边经 outline 层染色，文本色/光标走 `TextFieldColors`）；devOnly 下有测试屏（`SokitsuTestScreen` / `ButtonTestScreen` / `RadioButtonTestScreen` / `SliderTestScreen` / `TextFieldTestScreen` 等）与图集校验（`SokitsuAtlasValidation`）。
