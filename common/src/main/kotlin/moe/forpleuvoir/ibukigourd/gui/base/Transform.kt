 ...
 */
inline fun GuiWidget.mouseHover(block: GuiWidget.() -> Unit) {
    contract {
        callsInPlace(block, InvocationKind.AT_MOST_ONCE)
    }
    if (mouseHover()) block()
...
/**
 * 当鼠标位于此元素内容矩形[GuiWidget.contentBox]时调用
 * @receiver Element
 * @param block Element.() -> Unit
 */
inline fun GuiWidget.mouseHoverContent(block: GuiWidget.() -> Unit) {
    contract {
        callsInPlace(block, InvocationKind.AT_MOST_ONCE)
    }
 ...
 * @return Boolean
 */
fun GuiWidget.mouseHover(): Boolean = mc.mousePosition in transform
 ...
 * @return Boolean
 */
fun GuiWidget.mouseHover(mousePosition: MousePosition): Boolean = transform.isMouseOvered(mousePosition)

fun GuiWidget.mouseHoverContent(): Boolean = mc.mousePosition in contentBox(true)

fun GuiWidget.mouseHoverContent(mousePosition: MousePosition): Boolean = mousePosition in contentBox(true)
