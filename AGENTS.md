...
- **图集约定**：定义 `assets/<ns>/sokitsu_atlas/<atlasId>.json`；扫描目录 `texture/sokitsu/<atlasId>/`；纹理 id = 文件路径去掉 `texture/sokitsu/` 前缀与 `.aseprite` 后缀（如 `icon/add`）。现有图集 `ui` / `icon` / `sokitsu`。
- **组件**：`Button` / `ColorButton`（Button 薄包装：底色由调用方给定**具体颜色**而非主题槽位，内容色按底色亮度取纯黑/纯白，自有 `color_button` meta 段）/ `FlatButton`（**无常驻背景**的按钮底座：只在"当前状态有素材"时渲染背景，`normal` / `disabled` 无素材即整块透明（不能复用 `Surface`，其 `sprite=null` 会退化成纯色填充）；内容色按 meta 的 `contentBlend` 逐状态系数向黑白混合以拉大明暗差、禁用态按 `disabledBlend` 向背景混合以弱化；`IconButton` / `TextButton` 基于它，各有独立 meta 段）/ `Icon` + `Icons`（**像素图标集**：36 个 `.aseprite` 于 `texture/sokitsu/icon/`，独立图集 `icon`；`Icons.Add` 式属性常量 + `all` / `byId`，`Icon` 尺寸 = 素材尺寸 × `scale`（缺省 `pixelScale`），`tint` 缺省跟随 `LocalContentColor` 故在按钮内自动吃状态修正）/ `Surface` / `Switch` / `Text` / `Slider` / `RadioButton`（Button 薄包装：按 index/count/RTL 解析 left/center/right/single 分段纹理，`RadioButtonGroup` 经 `item { }` DSL 自动编号）/ `RadioButtonGroup`；`Slider` 为「凹槽轨道 + 按进度裁剪的填充」两张精灵叠放，`label` 槽内容画两遍、各裁到进度边界内外以分用两种内容色；`NumberSlider.kt` 在 Slider 之上提供类型化入口（Int/Long/Float/Double/Duration/Percent，双向换算 + 数值标签）；`TextField` 为凹槽背景精灵 + `BasicTextField`（聚焦/错误描边经 outline 层染色，文本色/光标走 `TextFieldColors`）；devOnly 下有测试屏（`SokitsuTestScreen` 主菜单 + `ButtonTestScreen` / `ColorSchemeTestScreen` / `FlatButtonTestScreen` / `IconTestScreen` / `IconButtonTestScreen` / `RadioButtonTestScreen` / `SliderTestScreen` / `TextFieldTestScreen` 等）与图集校验（`SokitsuAtlasValidation`）。
