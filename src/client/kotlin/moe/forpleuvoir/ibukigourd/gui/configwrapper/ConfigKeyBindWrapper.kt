...
fun WidgetContainerScope.ConfigKeyBindWrapper(
    ...
    val keyBindValue = mutableStateOf(config.getValue()).apply {
        subscribe {
            config.setValue(it)
        }
    }
    ...
    Column(
        ...
        Button(
            modifier = Modifier
                ...
                .keyPress { event ->
                    ...
                    if (inputting) {
                        inputting = false
                        config.getValue().setKey(*keys.toTypedArray())
            ...
            val setting = keyBindSetting().apply {
                copyFrom(config.getValue().setting)
            }
            PopupTip(
                settingsPopupState,
                screenModifier = Modifier.onClose {
                    config.getValue().setting.copyFrom(setting)
                    config.onChange(config)
                },
        ...
        ConfigResetButton(config, keyBindValue) {
            text.setValue(config.getValue().asText)
        }
    }
}