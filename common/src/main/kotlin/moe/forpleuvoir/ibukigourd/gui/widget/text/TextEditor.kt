...
open class TextEditorWidget(
    var textColor: ARGBColor = Color.ofARGB(0xFF303030),
    var hintColor: ARGBColor = Color.ofARGB(0xFF707070),
    var bgShaderColor: ARGBColor = Colors.WHITE,
    var selectedColor: ARGBColor = Color.ofARGB(0xFF007F8F).alpha(0.45f),
    var suggestionColor: ARGBColor = Color.ofARGB(0xFF008F72).alpha(0.45f),
    ...
    var selectionEnd: Int = 0
        private set(value) {
            ...
            val string = font.plainSubstrByWidth(text.substring(firstCharacterIndex), width)
            val k = string.length + firstCharacterIndex
            if (field == firstCharacterIndex) {
                firstCharacterIndex -= font.plainSubstrByWidth(text, width, true).length
            }
            if (field > k) {
    ...
    private fun write(text: String, historyOpt: Boolean = false) {
        ...
        if (length < StringUtil.filterText(text).also { string = it }.length.also { l = it }) {
    ...
    private fun getCursorPosWithOffset(offset: Int): Int {
        return Util.offsetByCodepoints(text, selectionStart, offset)
    }
    ...
    override fun onKeyPress(event: KeyPressEvent) {
        ...
        event.tryUse {
            ...
            if (InputHandler.wasKeyPressed(Keyboard.LEFT_CONTROL, Keyboard.V)) {
                write(mc.keyboardHandler.clipboard)
                return@tryUse true
            }
            //剪切选中文本
            ...
            return@tryUse when (event.keyCode) {
                ...
                Keyboard.LEFT      -> {
                    ...
                    moveCursor(Whence.RELATIVE, offset)
                    true
                }
    ...
    override fun onCharTyped(event: CharTypedEvent) {
        if (!isActive) return
        event.tryUse { StringUtil.isAllowedChatCharacter(Char(event.codepoint)) }
            .onSuccess {
    ...
    private fun setCursorFromMouse(mouseX: Float) {
        ...
        val str = font.plainSubstrByWidth(text.substring(firstCharacterIndex), contentWidth.toInt())
        val xOffset = (mouseX - contentLeft(true)).coerceAtLeast(0f)
        val count = font.plainSubstrByWidth(str, xOffset.toInt()).length
    ...
    fun renderCursor(guiGraphics: IGGuiGraphics) {
...
) = addWidgetChild(TextEditorWidget(textColor, hintColor, bgShaderColor, selectedColor, suggestionColor, cursorColor, font)) {
    Modifier
        .name("TextEditor")
        .mouseOverCursor(MouseCursor.IBEAM_CURSOR)
        .padding(5, 5, 5, 4)
        .renderBackground { guiGraphics, _, _, _ ->
            guiGraphics.pushWidgetTexture(transform, theme(WidgetTheme.TextInput), bgShaderColor)
        }
...
fun <T> ContainerScope.NumberEditor(
    ...
    TextEditor(
        ...
        textColor, hintColor, bgShaderColor, selectedColor, suggestionColor, cursorColor, font
    ) {
