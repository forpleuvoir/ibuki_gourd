...
interface ColoredVertex : Vector3fc, Serializable {
    ...
    companion object : Deserializer<ColoredVertex> {
        ...
        operator fun invoke(x: Number, y: Number, z: Number, color: ARGBColor): ColoredVertex = ColoredVertexImpl(x.toFloat(), y.toFloat(), z.toFloat(), color)

        operator fun invoke(vector3fc: Vector3fc, color: ARGBColor): ColoredVertex = ColoredVertexImpl(vector3fc, color)

        operator fun invoke(vector2fc: Vector2fc, color: ARGBColor): ColoredVertex = ColoredVertexImpl(vector2fc.x(), vector2fc.y(), 0f, color)
