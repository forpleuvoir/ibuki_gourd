...
fun interface WidgetModifier : Modifier.Element {
    fun applyModify(element: GuiWidget)

    override fun tryApplyModify(target: Any) {
        if (target is GuiWidget) applyModify(target)
    }
}

fun interface WidgetContainerModifier : Modifier.Element {
    fun applyModify(element: GuiWidgetContainerImpl)

    override fun tryApplyModify(target: Any) {
        if (target is GuiWidgetContainerImpl) applyModify(target)
    }
}
...
fun Modifier.renderBackground(action: GuiWidget.(IGGuiGraphics, Float, Float, Float) -> Unit) = this then WidgetModifier { drawable ->
    drawable.renderBackground = { context, mouseX, mouseY, delta ->
        drawable.action(context, mouseX, mouseY, delta)
    }
}

fun Modifier.render(action: GuiWidget.(IGGuiGraphics, Float, Float, Float) -> Unit) = this then WidgetModifier { drawable ->
    drawable.render = { context, mouseX, mouseY, delta ->
        drawable.action(context, mouseX, mouseY, delta)
    }
}

fun Modifier.renderOverlay(action: GuiWidget.(IGGuiGraphics, Float, Float, Float) -> Unit) = this then WidgetModifier { drawable ->
...
fun Modifier.measureCompletion(action: GuiWidget.() -> Unit) = this then WidgetModifier { widget ->
    widget.measureCompletion = { widget.action() }
}

//------------ Layout ------------\\

fun Modifier.layoutCompleted(action: GuiWidgetContainerImpl.() -> Unit) = this then WidgetContainerModifier { widget ->
...
fun Modifier.placeCompletion(action: GuiWidget.() -> Unit) = this then WidgetModifier { widget ->
    widget.placeCompletion = { action(widget) }
}

//------------ UserData ------------\\

fun <W : GuiWidget> Modifier.mouseOverCursor(mapping: MouseCursorMapping<W>) = this then WidgetModifier { widget ->
    runCatching {
