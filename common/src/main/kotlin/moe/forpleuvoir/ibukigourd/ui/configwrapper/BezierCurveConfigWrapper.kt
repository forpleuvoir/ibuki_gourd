 ...
 * 摘要本身悬停时用气泡画出当前曲线（只读，不响应拖拽）。
 *
 * 浮层以**可编辑副本**承载编辑：打开时复制当前值，确认时才写回配置，取消 / 遮罩 / Esc 关闭
 * 直接丢弃副本；需要恢复默认值时用行尾的重置按钮。
 *
 * @param config 曲线配置项
 * @param modifier 作用于整行
 * @param yRange 画布的 y 轴显示区间
 */
@Composable
fun BezierCurveConfigWrapper(
        ...
        ConfigControlBlock(
            ...
            Surface(
                modifier = Modifier
                    ...
                    // 悬停数值摘要时把当前曲线画出来：文本摘要看不出形状，气泡里直接给画布
                    .tooltip(delay = ConfigRowDefaults.TooltipDelay) {
                        CurveTooltipPreview(value, yRange)
                    },
    ...
    if (editing) {
        // 打开时复制当前值：编辑只改副本，确认时才写回配置，关闭即丢弃
 ...
 *
 * 列表本体与单条目完整编辑器都编辑副本：取消 / 关闭即丢弃，各自确认时才把副本写回上一层。
 */
@Composable
    ...
    FlexibleDialog(
        ...
        content = {
            EditDialogContent(
                ...
                // 单张卡片的完整编辑器：叠在卡片浮层之上，打开时复制该条目，确认时才写回副本列表
                val target = editingKey
                val index = target?.let { key -> keyed.entries.indexOfFirst { it.key == key } } ?: -1
                if (target != null && index >= 0) {
                    var draft by remember(target) { mutableStateOf(keyed.entries[index].value) }
                    FlexibleDialog(
                        onDismissRequest = { editingKey = null },
                        onConfirmRequest = {
                            val current = keyed.entries.indexOfFirst { it.key == target }
                            if (current >= 0) keyed.setValue(current, draft)
                            true
                        },
 ...
 *
 * 尺寸取 [ConfigControlDefaults.CurveTooltipPreviewSize]；曲线行与缓动选择行共用。
 *
 * @param value 要预览的曲线
 * @param yRange 画布的 y 轴显示区间
 */
@Composable
internal fun CurveTooltipPreview(value: CubicBezier, yRange: ClosedFloatingPointRange<Float>) {
    BezierCurvePlot(
        value = value,
        onValueChange = {},
        interactive = false,
        yRange = yRange,
        modifier = Modifier.size(ConfigControlDefaults.CurveTooltipPreviewSize),
    )
}
