...
class TextInput(private val textRenderer: TextRenderer = moe.forpleuvoir.ibukigourd.util.textRenderer) : ClickableElement() {
    ...
    fun renderText(renderContext: RenderContext) {
        ...
        //"渲染选中的文本高亮"
        val (startIndex, endIndex) = (this.selectionStart - this.firstCharacterIndex).coerceAtLeast(0) to (this.selectionEnd - this.firstCharacterIndex).coerceAtLeast(0)
        if (this.selectedText.isNotEmpty() || focused) {
