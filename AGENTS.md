...
| `lang` | `IGLang`（i18n 键命名空间中心）、`MiscLang` / `ColorLang` / `ConfigWrapperLang` / `InputLang` / `ThemeLang`、`TranslationRecorder` |
| `mod` / `mod.config` | IbukiGourd 自身内容：配置 `IGConfig`（`Gui` 组下 `Theme` / `Toast` / `Scroller` / `Screen` / `Dialog`，以及 `quickActionKeyCode` / `hideActionKeyCode` / `openScreen`；`Screen` 组含解除界面内 60 帧上限的 `unlimit_framerate` 与作为 `SokitsuScreen.pauseGame` 缺省值的 `pause_game`，前者经 `ui/ComposeScreenHelper.kt` 供给每帧查询它的 `mixin/client/FramerateLimitTrackerMixin.java`）+ `ThemeConfig.kt`（`ThemeMode` / `resolveThemeScheme` / `CustomColorSchemeEditor`，即 `Gui.Theme` 配色编辑器的接线）；自身屏幕 `IbukiGourdModScreen.kt`（`ibukiGourdModScreen` / `openIbukiGourdModScreen`：顶栏 + 彩蛋触发 + 页签条，**配置页是其中一页**；ModMenu / NeoForge 模组列表的"配置"按钮也开它） |
...
| `ui`（根） | `ModScreen.kt` —— **公共模组屏幕 API**：`ModScreen`（两个重载：缺省顶栏 = `title` / `icon` / `headerActions` 三个可组合槽位，或整个顶栏由调用方给）+ `ModScreenTab` / `ModScreenState` / `rememberModScreenState` / `ModScreenHeader` / `ModScreenIcon` / `ModScreenDefaults`；页签条走 `TabStrip`（上朝向）铺主题全屏面板素材，不绑定具体模组；同包的 `ComposeScreenHelper.kt` 只回答「当前屏幕是不是 `ComposeScreen`」并给原版补丁提供界面帧率开关 |
| `ui.configwrapper` | 配置 GUI（**已重建**，作为模组屏幕 `mod/IbukiGourdModScreen.kt` 的一页）：页面骨架 `ConfigManagerWrapper`（搜索栏 + 分组导航）/ `ConfigGroupWrapper` / 行骨架 `ConfigRowWrapper`（名称 + 控件 + 重置 + tooltip）/ `ConfigUIWrapper` + `UIWrappers` 类型注册表（按谓词分发整行，节点可用 `uiWrapper` 指定专用实现）/ `ConfigElementEditor`（编辑弹窗内按值的运行时类型取控件，不带行骨架）/ `ConfigRowTheme` / `ConfigState`（`Config` → Compose `State`）；各类型 wrapper：`Primitive`(Bool/Int/Long/Float/Double) / `String` / `Enum` / `Duration` / `Vector` / `Color` / `Keybind` / `List` / `Map` / `BezierCurve` / `Easing`（缓动预设行：选中「自定义」即弹曲线编辑器，悬停显示当前自定义曲线的预览）；页面导航走 `TabRow` 页签 + 底部搜索栏 |
...
| `ui.overlay` | **全局覆盖层**：`OverlayService`（注册表：`register(key, present, content)` / `registerFrameListener` → `OverlayHandle`，`unregister` / `contains` / `clear`；条目是 snapshot state，`present` 为 false 的条目连组合都不参与）+ `OverlayHost`（**非屏幕**的自持 `MinecraftComposeScene`，场景根复用 `SokitsuScreenRoot`，所以覆盖层与屏幕**同档缩放**（同一 `SokitsuScreenDefaults.resolver`）；由 `mixin/client/GuiRendererOverlayMixin.java` 挂在 `GuiRenderer.render` TAIL 每帧驱动：推进帧回调 → 只转发鼠标 MOVE → 渲染提交；空闲帧零开销，渲染器不抢 `ComposeGuiRenderer.active` 单值槽位；条目内再套主题换配色时要接回 `LocalSokitsuPixelScale.current`）+ `OverlayContainer`（按注册顺序组合条目，铺满窗口）；内容是画在原版 HUD / 原版界面 / Compose 屏幕**之上**的只读覆盖物（要交互请做屏幕或弹层），`Toast` 即它的一个条目 |
...
| `ui.curve` | 缓动曲线编辑器：`BezierCurveEditor` / `BezierCurvePlot`(+Theme)（预设行 + 四点编辑 + 曲线预览，被 `BezierCurveConfigWrapper` 与 `EasingConfigWrapper` 消费） |
| `ui` / `ui.sokitsu{,.theme,.draw,.texture{,.atlas},.menu,.toast,.tooltip}` | 新 UI 体系（Sokitsu，像素风）**组件**：`Button` / `ColorButton` / `FlatButton`(+`IconButton` / `TextButton`) / `Icon`(+`Icons`) / `Surface` / `Switch` / `Slider`(+`NumberSlider`) / `TextField`(+`NumberField`) / `Text` / `Divider` / `ProgressBar` / `RadioButton`(+`RadioButtonGroup`) / `Scroller`（常规 + flat 细条，`autoHide` / `autoFade`；滚动源含 `LazyListState` / `LazyGridState` 适配器）/ `TabStrip` / `TabRow`（等宽页签行 + 选中指示器） / `TableLayout`(+`LazyTableLayout`：列宽支持权重与上下限、固定表头排在列表之外、`listTrailing` 槽位) / `AlertDialog`(+`SimpleAlertDialog`) / `FlexibleDialog`（`screenPadding` / `maxHeight`）（配 `*Theme` 组件 token 取色，颜色走「调用点传参 > 组件 token 表 > 主题槽位」回退链）；**屏幕**：`SokitsuScreen`（`ComposeScreen` 薄包装，套 `SokitsuTheme` + `SokitsuScreenRoot` 并按窗口分辨率选 `SokitsuScreenScale`，同时下发 `LocalDensity` 与 `pixelScale`）；**交互状态**：`UiState`（`disabled > pressed > (hover ∪ focused) > normal`）；`theme`：`SokitsuTheme` 入口、`ColorScheme` + `ColorSchemeToken`（语义槽位）与 `TokenResolution`（回退链解析）、`ContentColor` / `Typography` / `SokitsuIndicationNodeFactory` / `SokitsuTextSelectionColors`、`ThemeType` + `systemTheme()`、`SokitsuThemeMeta(+Loader)` 资源包主题 meta、`SokitsuThemeOverride`（配置驱动的运行时配色覆盖）、`ColorSchemeEditor`（`ColorSchemeEditorDialog`：槽位按钮组 + 亮暗标记 + 以默认浅/深色为底）；`draw`：`sokitsuSprite` 精灵绘制 + `BubblePanel` 气泡体及其渲染插件；`texture.atlas`：运行时程序化图集（`SokitsuAseLoader` → `SokitsuStitcher` → `SokitsuAtlasManager`）；`menu`：`DropdownMenu`(+`DropdownMenuItem`) / `SokitsuContextMenu`；`toast`：全局常驻提示，实现为 `ui.overlay` 的一个注册条目（经 `ToastOverlay.install()` 装进覆盖层，随 `GuiRendererOverlayMixin` 每帧驱动）；`tooltip`：`Tooltip` / `BasicTooltip`（经 `LocalPopupHost` 注册到场景根，**不产生布局节点**） |
...
旧的 Compose Desktop 离屏渲染 UI（`preset` / `icon` / `scene` / `skia` / `widget` 等子包，以及 `mod/ui/` 屏幕、旧 `mod/waht/` 彩蛋游戏实现）已随迁移移除，对应的旧 `ModScreen` / `SkiaContext` 等 API 一并删除；`ui/overlay` 已按新基座重建（见包结构表），`ui/configwrapper/` 也是基于 sokitsu 组件**重建的新实现**，与迁移前的同名包无关。
...
- **浮层与提示**：`DropdownMenu`(+`DropdownMenuItem`)（无状态 API + 便利 `DropdownMenuState`，锚点经 `Modifier.dropdownMenuAnchor` 捕获，面板用共享 `BubblePanel`）/ `SokitsuContextMenu`（由 `SokitsuTheme` 注入 `LocalContextMenuRepresentation`）/ `Toast`（全局常驻，是 `ui.overlay` 覆盖层的注册条目，经 `GuiRendererOverlayMixin` 每帧驱动，HUD / 原版界面 / Compose 屏幕之上都可见）/ `Tooltip` + `BasicTooltip`（经 `LocalPopupHost` 注册到场景根，**不产生布局节点**，`pinned` 时跳过悬停延迟直接展示）/ `UiState`（`disabled > pressed > (hover ∪ focused) > normal`）。
