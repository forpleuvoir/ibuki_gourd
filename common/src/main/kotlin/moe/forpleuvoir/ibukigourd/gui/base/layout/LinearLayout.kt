...
interface ColumnLayout : LinearLayout<Arrangement.Vertical, Alignment.Horizontal> {
    ...
    override fun measureChildren(measurables: List<Measurable>, constraints: Constraints): Placeable {
        ...
        //总权重
        val totalWidget = parentData.sumOf { it.weight }

        //优先计算固定最小尺寸的组件的占用
        var minimumOccupiedHeight = 0f
        measurables.forEachIndexed { index, child ->
            if (parentData[index].run { weight <= 0 && fillMode == FillMode.MatchSibling }) {
        ...
        if (totalWidget != 0) {
            ...
            measurables.forEachIndexed { index, child ->
                if (parentData[index].run { weight > 0 && fillMode != FillMode.MatchSibling }) {
                    val weight = parentData[index].weight
                    val distributionHeight = ((weightUnitHeight * weight) - child.margin.height).coerceIn(0f, child.constraints.maxHeight)
                    val placeable = child.measure(
                        Constraints.of(
                            if (parentData[index].fillMode == FillMode.MatchParent) contentMaxWidth - child.margin.width else 0f,
                            contentMaxWidth - child.margin.width,
    ...
    override fun layout(layoutables: List<Layoutable>) {
        ...
        val alignments = WrappedColumnLayoutData.wrappedData(layoutables).map { it.getAlignment(this) }
...
interface RowLayout : LinearLayout<Arrangement.Horizontal, Alignment.Vertical> {
    ...
    override fun measureChildren(measurables: List<Measurable>, constraints: Constraints): Placeable {
            ...
            if (parentData[index].run { weight <= 0 && fillMode != FillMode.MatchSibling }) {
                val placeable = child.measure(
                    Constraints.of(
                        ...
                        if (parentData[index].fillMode == FillMode.MatchParent) contentMaxHeight - child.margin.height else 0f,
        ...
        if (totalWidget != 0) {
            measurables.forEachIndexed { index, child ->
                if (parentData[index].run { weight > 0 && fillMode != FillMode.MatchSibling }) {
                    val weight = parentData[index].weight
                    val distributionWidth = ((weightUnitWidth * weight) - child.margin.width).coerceIn(0f, child.constraints.maxWidth)
                    val placeable = child.measure(
                        Constraints.of(
                            distributionWidth,
                            distributionWidth,
    ...
    override fun layout(layoutables: List<Layoutable>) {
        ...
        val alignments = WrappedRowLayoutData.wrappedData(layoutables, WrappedRowLayoutData()).map { it.getAlignment(this) }
