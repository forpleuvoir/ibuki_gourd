...
interface TableLayout : Layout {
    ...
    @Deprecated("", replaceWith = ReplaceWith("measureColumns(columns, constraints)"))
...
data class TableColumn(
    val column: List<GuiWidget>,
    val weight: Int = 0,
    val size: Int = column.size,
) {

    val cachedCells = arrayOfNulls<Pair<GuiWidget, Box>>(size)

    companion object {
        fun extractColumns(tableColumns: List<TableColumn>): List<List<GuiWidget>> {
            if (tableColumns.isEmpty()) return emptyList()
            val rowCount = tableColumns.first().column.size
            val result = List(rowCount) { mutableListOf<GuiWidget>() }
