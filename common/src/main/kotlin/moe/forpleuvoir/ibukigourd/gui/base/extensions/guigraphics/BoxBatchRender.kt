...
data class BoxBatchRenderScope internal constructor(val vertexConsumer: VertexConsumer, val matrix4f: Matrix4f) {
    ...
    fun pushBox(box: Box, color: ARGBColor) {
        for (vertex in box.vertexes) {
            vertexConsumer.vertex(matrix4f, vertex).color(color)
        }
    }
    ...
    fun pushBox(coloredBox: ColoredBox) {
        for (vertex in coloredBox.coloredVertexes) {
            vertexConsumer.vertex(matrix4f, vertex).color(vertex.color)
        }
    }
